...
export function LogCard({ log }: { log: Partial<Log> }) {
	...
	const renderParams = (
		...
		return Object.entries(obj).flatMap(([key, value]) => {
			...
			if (typeof value === "object" && !Array.isArray(value)) {
				return renderParams(value as Record<string, unknown>, depth + 1);
			}
	...
	return (
		<div className="rounded-lg border bg-card text-card-foreground shadow-sm max-w-full overflow-hidden">
			<div
				className={`flex items-start gap-4 px-5 py-4 ${isExpanded ? "border-b" : ""}`}
			>
				...
				<div className="flex-1 space-y-1 min-w-0">
					<div className="flex items-start justify-between gap-4">
						<div className="flex items-center gap-2 flex-1 min-w-0">
							<p className="font-medium wrap-break-word max-w-none line-clamp-2">
								{log.content ||
									(log.unifiedFinishReason === "tool_calls" && log.toolResults
										? Array.isArray(log.toolResults)
											? `Tool calls: ${(log.toolResults as { function?: { name?: string } }[]).map((tr) => tr.function?.name || "unknown").join(", ")}`
											: "Tool calls executed"
										: "---")}
							</p>
							{!log.content &&
				...
				<div className="space-y-4 px-5 py-4">
					<div className="grid gap-4 md:grid-cols-2">
						<div className="space-y-2">
							...
							<div className="grid grid-cols-2 gap-x-6 gap-y-2 rounded-md border p-4 text-sm">
							...
							{log.routingMetadata && (
								<div className="mt-3">
									...
									<div className="rounded-md border border-dashed p-3 text-xs space-y-1.5 bg-muted/30">
					...
					<div className="space-y-2">
						...
						<div className="grid gap-4 rounded-md border p-4 text-sm sm:grid-cols-2 md:grid-cols-4">
							<TooltipProvider>
								{isTts ? (
									<>
											...
											<span>
												{log.responseFormat
													? typeof log.responseFormat === "object"
														? (log.responseFormat as { type?: string }).type ||
															"-"
														: "-"
													: "-"}
											</span>
							...
							<div className="grid gap-4 md:grid-cols-1">
								{log.tools && (
									<div className="space-y-2">
										...
										<div className="rounded-md border p-3">
											<pre className="max-h-40 text-xs overflow-auto whitespace-pre-wrap wrap-break-word">
					...
					<div className="space-y-2">
						...
						<div className="rounded-md border p-3">
							{log.messages ? (
								<pre className="max-h-60 text-xs overflow-auto whitespace-pre-wrap wrap-break-word">
