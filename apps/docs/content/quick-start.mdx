---
title: Quickstart
description: Fastest way to start using LLM API in any language or framework.
icon: Rocket
---
...
Welcome to **LLM API**—a single drop‑in endpoint that lets you call today’s best large‑language models while keeping **your existing code** and development workflow intact.

> **TL;DR** — Point your HTTP requests to `https://internal.llmapi.ai/v1/…`, supply your `LLM_API_KEY`, and you’re done.
...
```bash
export LLM_API_KEY="llmgtwy_XXXXXXXXXXXXXXXX"
```
...
>
  <Tab value="cURL">
    <DynamicCodeBlock lang="bash" code={`curl -X POST https://internal.llmapi.ai/v1/chat/completions \\
  ...
  <Tab value="TypeScript">
    <DynamicCodeBlock lang="typescript" code={`const response = await fetch('https://internal.llmapi.ai/v1/chat/completions', {
  method: 'POST',
  headers: {
...
setLoading(true);
try {
const res = await fetch('https://internal.llmapi.ai/v1/chat/completions', {
method: 'POST',
headers: {
'Content-Type': 'application/json',
...
const response = await fetch('https://internal.llmapi.ai/v1/chat/completions', {
method: 'POST',
headers: {
'Content-Type': 'application/json',
'Authorization': \`Bearer \${process.env.LLM_API_KEY}\`
...
.uri(URI.create("https://internal.llmapi.ai/v1/chat/completions"))
...
let api_key = env::var("LLM_API_KEY")?;
    ...
    let response = client
        .post("https://internal.llmapi.ai/v1/chat/completions")
        .header("Content-Type", "application/json")
...
apiKey := os.Getenv("LLM_API_KEY")
    ...
    req, _ := http.NewRequest("POST", "https://internal.llmapi.ai/v1/chat/completions", bytes.NewBuffer(jsonData))
...
uri = URI('https://internal.llmapi.ai/v1/chat/completions')
...
const completion = await llmapi.chat({
	model: "gpt-4o",
...
<Accordions type="single">
	<Accordion title="Which models are supported?">
		See the [Models page](https://app.llmapi.ai/models).
	</Accordion>
	...
	<Accordion title="How much do you charge for your services?">
		...
		[Pricing section](https://llmapi.ai#pricing).
	</Accordion>
</Accordions>
...
- Drop into our [GitHub](https://github.com/Spendbase/llmgateway) for help or feature requests.
