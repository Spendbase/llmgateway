...
description: Use LLM API with Cline for AI-powered coding assistance in VS Code
...
[Cline](https://cline.bot) is an autonomous AI coding assistant that lives in your VS Code editor. It can create and edit files, run terminal commands, and help you build complex projects. You can configure Cline to use LLM API for access to multiple AI providers with unified billing and cost tracking.
...
Cline supports OpenAI-compatible API endpoints, making it straightforward to integrate with LLM API.
...
2. Enter the following details:
   ...
   - **Model ID**: Choose a model (e.g., `claude-sonnet-4-6`, `claude-opus-4-6`, `claude-opus-4-5-20251101`, `gpt-5.2`, `gemini-3-pro-preview`, `deepseek-3.2`). See [provider-specific routing](/features/routing#provider-specific-routing) for more options.
...
All requests will now be routed through LLM API.
...
Once configured, you can use all of Cline's features with LLM API:
...
LLM API offers discounted access to some models. Find them on the [models page](https://llmapi.ai/models?view=grid&filters=1&discounted=true) and copy the model ID.

### Using Free Models

Some models are available for free. Browse them on the [models page](https://llmapi.ai/models?view=grid&filters=1&free=true).
...
## Benefits of Using LLM API with Cline
