...
description: Use LLM API with Cursor IDE for AI-powered code editing and chat
...
Cursor is an AI-powered code editor built on VSCode. You can configure Cursor to use LLM API for enhanced AI capabilities, access to multiple models, and better cost control.

![Cursor with LLM API](/guides/cursor/overview.png)
...
Cursor supports OpenAI-compatible API endpoints, making it easy to integrate with LLM API.

<Steps>
<Step>
### Get Your API Key

1. Log in to your [LLM API dashboard](https://llmapi.ai/dashboard)
2. Navigate to **API Keys** section
3. Create a new API key and copy the key

![LLM API API Keys](/guides/cursor/api-key.png)
...
2. Choose any [LLM API supported model](https://llmapi.ai/models):
...
- For custom models: Add the provider name before the model name (e.g.
  `custom/my-model`)
- For discounted models: copy the ids from from the [models page](https://llmapi.ai/models?view=grid&filters=1&discounted=true)
- For free models: copy the ids from from the [models page](https://llmapi.ai/models?view=grid&filters=1&free=true)
- For reasoning models: copy the ids from from the [models page](https://llmapi.ai/models?view=grid&filters=1&reasoning=true)
...
All AI requests will now be routed through LLM API.
...
Once configured, you can use all of Cursor's AI features with LLM API:
...
6. **Reasoning Model**: Use a reasoning model like `canopywave/kimi-k2-thinking` [with 75% off discount](https://llmapi.ai/changelog/canopywave-kimi-k2-thinking-discount)

This gives you the best balance of performance and cost.

### Model Routing

With LLM API's [routing features](/features/routing), you can:
...
- Check that the base URL is set to `https://internal.llmapi.ai/v1`
- Ensure your LLM API account has sufficient credits

### Model Not Found

If you see "model not found" errors:

- Verify the model ID exists in the [models page](https://llmapi.ai/models)
- Check that you're using the correct model name format
- Some models may require specific provider configurations in your LLM API dashboard
...
- Monitor your usage in the LLM API dashboard
- Consider using faster models for autocomplete features

<Callout type="info">
	Need help? Join our [Discord community](https://llmapi.ai/discord) for support
	and troubleshooting assistance.
</Callout>

## Benefits of Using LLM API with Cursor
