...
LLM API supports image generation models through the standard chat completions API. These models can create images based on text prompts and return them as base64-encoded data URLs.

## Available Models

You can find all available image generation models on our [models page](https://llmapi.ai/models?filters=1&imageGeneration=true).
...
export async function POST(req: Request) {
	...
	try {
		const result = streamText({
			model: llmapi.chat("gemini-2.5-flash-image-preview"),
		...
		return new Response(
			JSON.stringify({ error: "LLM API Chat request failed" }),
			{
				status: 500,
			},
		);
