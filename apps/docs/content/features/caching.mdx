...
LLM API provides intelligent response caching that can significantly reduce your API costs and response latency. When caching is enabled, identical requests are served from cache instead of making redundant calls to LLM providers.

## How It Works

When you make an API request:

1. LLM API generates a cache key based on the request parameters
...
<Callout type="warning">
	...
	"Retain All Data" level. This allows LLM API to store and retrieve response
	payloads.
</Callout>
