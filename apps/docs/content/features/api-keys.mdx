...
API keys are the primary method for authenticating with the LLM API. This guide covers creating API keys, managing them, and configuring IAM rules for fine-grained access control.

## Overview

LLM API provides comprehensive API key management with the following features:
...
1. Navigate to your project in the LLM API dashboard
...
Usage is tracked per API key which is shown on the API Keys page. Usage includes both costs from LLM API credits and usage from your own provider keys when applicable, giving you complete visibility into total spending per key.
