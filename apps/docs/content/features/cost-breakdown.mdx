...
LLM API provides real-time cost information for each API request directly in the response's `usage` object. This allows you to track costs programmatically without needing to query the dashboard.

<Callout type="info">
	Cost breakdown is available for **Pro plan** users on hosted LLM API.
...
| `cost_usd_data_storage` | LLM API storage cost in USD ($0.01 per 1M tokens, only when retention enabled) |

<Callout type="info">
	**Note:** `cost_usd_total` includes only provider/inference costs. Data
	storage costs (`cost_usd_data_storage`) are billed separately by LLM API when
	data retention is enabled in organization policies.
</Callout>
...
If you're on the free plan (hosted LLM API), the `usage` object will include an informational message instead of cost fields:
...
<Callout type="success">
	[Upgrade to Pro](https://app.llmapi.ai/dashboard) to unlock real-time cost
...
When data retention is enabled in organization policies, LLM API stores full request and response payloads for the configured retention period. This storage incurs a small additional cost:
...
Storage costs are displayed separately from inference costs in the dashboard and usage breakdown to maintain transparency between provider costs and LLM API platform costs.
...
If you're running a self-hosted LLM API deployment, cost breakdown is always included in API responses regardless of plan. This allows you to track internal costs and allocate them across teams or projects.
