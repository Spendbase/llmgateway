...
Response Healing is a plugin that automatically validates and repairs malformed JSON responses from AI models. When enabled, LLM API ensures that API responses conform to your specified schemas even when the model's formatting is imperfect.
...
When Response Healing is enabled, LLM API applies a series of repair strategies to malformed JSON responses:
