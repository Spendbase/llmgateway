...
description: Connect Azure to LLM API for enterprise-grade OpenAI models
icon: Cloud
---

import { Step, Steps } from "fumadocs-ui/components/steps";

Azure provides access to OpenAI's powerful language models through Microsoft's enterprise cloud infrastructure. This guide shows how to create an Azure resource, deploy models, and integrate them with LLM API.

<Callout type="info">
	Only OpenAI models are supported via Azure at this time. [Open an
	issue](https://github.com/Spendbase/llmgateway/issues/new) to request support
...
- LLM API account (Pro plan required for provider keys) or self-hosted instance (free)

## Overview

Azure provides enterprise-grade access to OpenAI models with enhanced security, compliance, and regional availability. LLM API integrates seamlessly with Azure deployments.
...
1. Log into [LLM API Dashboard](https://llmapi.ai/dashboard)
...
curl -X POST https://internal.llmapi.ai/v1/chat/completions \
  -H "Authorization: Bearer YOUR_LLMAPI_API_KEY" \
...
Replace `YOUR_LLMAPI_API_KEY` with your LLM API API key.
...
Once configured, you can access your Azure deployments through LLM API:

- **GPT-4o**: `azure/gpt-4o`
- **GPT-4o Mini**: `azure/gpt-4o-mini`
- **GPT-3.5 Turbo**: `azure/gpt-3.5-turbo` (note: use gpt-3.5-turbo as llmapi model name instead of gpt-35-turbo)
...
Browse all available models at [llmapi.ai/models](https://llmapi.ai/models?provider=azure)
