...
import { formatTokenPrice } from "./model-detail-formatters";

import type { Model, ModelProviderMapping } from "@/lib/models";

function TextMappingsTable({
	mappings,
	modelName,
}: {
					...
					<tr key={mapping.id} className="hover:bg-muted/30 transition-colors">
							...
							<div className="space-y-1 text-xs">
								<div className="flex items-center gap-1">
									...
									<span className="text-muted-foreground">
										In: {formatTokenPrice(mapping.inputPrice)}/1M
									</span>
								</div>
								<div className="flex items-center gap-1">
									<DollarSign className="h-3 w-3 text-muted-foreground" />
									<span className="text-muted-foreground">
										Out: {formatTokenPrice(mapping.outputPrice)}/1M
									</span>
								</div>
						...
						<td className="px-4 py-4">
							<div className="space-y-1 text-xs text-muted-foreground">
								{mapping.contextSize && (
									<div>Context: {(mapping.contextSize / 1000).toFixed(0)}K</div>
								)}
								{mapping.maxOutput && (
									<div>Max out: {(mapping.maxOutput / 1000).toFixed(0)}K</div>
								)}
							</div>
						</td>
						<td className="px-4 py-4">
							<div className="flex flex-wrap gap-1">
								{mapping.streaming && <Badge variant="default">Stream</Badge>}
								{mapping.vision && <Badge variant="info">Vision</Badge>}
								{mapping.reasoning && <Badge variant="purple">Reasoning</Badge>}
								{mapping.tools && <Badge variant="default">Tools</Badge>}
								{mapping.jsonOutput && <Badge variant="default">JSON</Badge>}
								{mapping.webSearch && <Badge variant="default">Web</Badge>}
							</div>
						</td>
						<td className="px-4 py-4">
...
export function TextModelDetail({ model }: { model: Model }) {
		...
		<>
			...
			<div className="rounded-xl border border-border/60 bg-card shadow-sm overflow-hidden">
				...
				<div className="overflow-x-auto">
					<TextMappingsTable mappings={model.mappings} modelName={modelName} />
				</div>
			</div>
		</>
	);
