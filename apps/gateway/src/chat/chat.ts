...
chat.openapi(completions, async (c) => {
	...
	if (!noFallback && usedProvider && requestedProvider) {
	...
	if (!routingMetadata && usedProvider) {
	...
	// Apply organization context to messages if configured
	// This must happen before cache generation and request body preparation
	if (organization?.organizationContext) {
		applyOrganizationContext(messages, organization.organizationContext);
	}
